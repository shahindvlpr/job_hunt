            ...
            <div class="pending_request">
                ...
                <div class="request">
                    <table class="table table-bordered" style="margin-left:10px; margin-top:50px;" id="customers">
                        ...
                        <?php
                            $query = "SELECT * FROM `recruiter_complain`;";
                            $result = mysqli_query($conn,$query);
                            #$Pending = mysqli_fetch_array($result);
                            if($result->num_rows>0){
                                while($rd = mysqli_fetch_assoc($result)){
                                    $recruiter_id = $rd['recruiter_id'];
                                    $query06 = "SELECT * FROM job_recruiter WHERE recruiter_id = $recruiter_id;";
                                    $execute = mysqli_query($conn,$query06);
                                    $recruiterResult = mysqli_fetch_assoc($execute);
                                    $recruiter_name = $rd['user_name'];
                                    $email = $rd['email'];
                                    $company_name = $recruiterResult['company_name'];
